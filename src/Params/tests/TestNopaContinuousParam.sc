TestNopaNopaContinuousParam : TestNopaAbstractParamMeta {
  var param;

  setUp {
    param = NopaContinuousParam(5, 10, \lin, 0.1);
  }
  ...
  test_midi {
    ...
    this.assertFloatEquals(param.normalizeMidi(10), 0.078740157480315, "Normalize MIDI");
    this.assertFloatEquals(param.normalizeMidi(-10), 0, "Enforce MIDI lower bounds");
    this.assertFloatEquals(param.normalizeMidi(140), 1, "Enforce MIDI upper bounds");

    this.assertFloatEquals(param.midiUnmap(7.5), 63.5, "Unmap MIDI");
    this.assertFloatEquals(param.midiMap(63.5), 7.5, "Map MIDI");
  }
