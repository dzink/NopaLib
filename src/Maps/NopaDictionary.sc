...
NopaDictionary : IdentityDictionary {
  ...
  *newFrom {
  ...
  put {
    arg key, value;
    if (value.isKindOf(NopaAbstractParam)) {
      super.put(key, value);
    } {
      ("Only NopaAbstractParams and subclasses can be added to a NopaDictionary.")
    };
  }
  ...
  asPreset {
    arg args = nil;
    ^ NopaPreset.newFrom(this, args: args);
  }
  ...
  compareToSynthDefs {
    ...
    if (param.size.asBoolean) {
      ("In NopaDictionary but not in SynthDefs:").warn;
      (param.asString).warn;
    };
    if (sd.size.asBoolean) {
      ("In SynthDefs but not in NopaAbstractParam:").warn;
      (sd.asString).warn;
    };
    ^[param, sd];
  }
  ...
  randomize {
    ...
    args.do {
      arg key;
      if (this.at(key).isKindOf(NopaContinuousParam)) {
  ...
  linkToNode {
    ...
    args.do {
			arg key;
      [key, node].postln;
			this.at(key).addObserver(NopaNodeTarget(node, key));
